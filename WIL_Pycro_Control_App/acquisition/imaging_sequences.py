...
class ImageAcquisition(ABC):
    ...
    def close_datastore(self):
        """
        closes datastore at self._datastore
        """
    ...
    def _set_imaging_mode(self, exposure):
        camera.default_mode(exposure)
    ...
    @abstractmethod
    def _pop_image_with_metadata(channel_num: int, current_frame: int):
        """
        pops the next image from the MM circular buffer, grabs its metadata, adds additional properties, and then
        returns image with the updated metadata.
        """
        pass
...
class SequenceAcquisition(ImageAcquisition, ABC):
    ...
    def _acquire_images(self, channels: list):
        ...
        for channel_num, channel in enumerate(channels):
            ...
            self._acq_directory.set_acq_type(f"{self._get_name()}/{channel}")
            self._datastore = pycro.multipage_datastore(self._acq_directory.get_directory())
            self._set_summary_metadata(channel)
    ...
    def _wait_for_images(self, channel_num: int):
        ...
        is_saving is set to True when sequence acquisition is over but there are still images to be saved.
        """
        current_frame = 0
        time_no_image = 0
        is_saving = False
...
class Snap(SnapAcquisition):
    ...
    def _acquire_images(self):
        self._abort_check()
        for channel_num, channel in enumerate(self._region.snap_channel_list):
            self._abort_check()
    ...
    def run(self):
        self._set_imaging_mode(self._region.snap_exposure)
...
class Video(SequenceAcquisition):
    ...
    def run(self):
        self._set_imaging_mode(self._region.video_exposure)
...
class ZStack(SequenceAcquisition):
    ...
    def _initialize_z_stack(self):
        ...
        stage.scan_setup(self._region.z_stack_start_pos, self._region.z_stack_end_pos, self._adv_settings.z_stack_stage_speed)
...
class SpectralZStack(ZStack, SnapAcquisition):
    ...
    def run(self):
        self._set_imaging_mode(self._adv_settings.z_stack_exposure)
        for update_message in self._acquire_images():
            yield update_message
